...
**Code never lands on `main` as a direct commit.** A ticket that edits code runs in its own worktree and finishes as a pull request that has been reviewed on both axes, fixed, and squash-merged — the sequence is in `docs/agents/code-change-workflow.md`, and it is binding, not a suggestion.
...
- **Changing code** — the worktree, the gates, the PR, the two reviews, the merge: `docs/agents/code-change-workflow.md`.
