...
**Дата проверки:** 2026-08-23; 4-я причина в §0 добавлена 2026-08-24.

> **Прогон шёл на `bdd3f1d`.** Уже после его старта [PR #70](https://github.com/Conte777/infra-mcp/pull/70) переписал `guard.go` на глобы: функции `calls()`, разобранной в §5.3, больше нет — её сменили `calledNames` и `applied`, обе покрыты на 100%, так что три найденные там границы отпали вместе с ней. Переизмерено на `21a4a23` и устояло: `copiesToProgram` — 42.9%, `scannable` — 66.7%. С [PR #76](https://github.com/Conte777/infra-mcp/pull/76) нет и `scannable` с `applied`: разбор уехал в `lex.go`. Номера строк `guard.go` из §5.3 требуют перепрогона, прежде чем на них ссылаться. Остальных разобранных здесь файлов #70 не касался.
...
**Не делать гейтом в CI.** Четыре причины, все проверены прогоном, не предположены. Четвёртая — самая грубая: на четырёх воркерах прогон дважды уронил ноутбук по нехватке памяти, и это про то, как запускать вообще, а не только про CI.
...
3. Интеграционный набор поднимает по контейнеру на тест. Под тремя-четырьмя воркерами докер начинает отваливаться по таймауту: у меня базовый прогон покрытия упал на `TestDescribeCannotBeMadeToForgeADDLLine` (`context deadline exceeded` при старте контейнера), и весь прогон gremlins оборвался с `ERROR: failed to gather coverage`.
   ...
   Оговорка: флап случился, когда параллельно шёл ещё один прогон с докером. Чистый прогон на трёх воркерах отработал 121 мутанта без единого таймаута.

4. **На четырёх воркерах прогон уронил машину — дважды подряд** (2026-08-24, Mac16,12, macOS 26.5.2, при работе над [#71](https://github.com/Conte777/infra-mcp/issues/71)). Не «стало медленно»: в отчёте о панике `panic: watchdog timeout: no checkins from watchdogd in 92 seconds` при `Compressor Info: 100% of compressed pages limit (BAD)` и 21 свопфайле. Компрессор виртуальной памяти упёрся в потолок, система встала на пейджинге, демоны перестали получать процессор, ядро уронило себя по watchdog-таймауту; второй раз машина зависла намертво и была перезагружена руками. Механика доказана отчётом; что виноват именно gremlins — сильная косвенная улика: оба падения случились только на его прогонах, а `task test` с `-race` по всему модулю и `task test:integration` с контейнером на тест после перезагрузки прошли спокойно. Логи за окно перед паникой не записались — `logd` стоял вместе со всем остальным, так что назвать потребителя памяти по имени нечем.

   Механизм у этого простой: каждый мутант — отдельный `go test`, то есть полная компиляция и **линковка** пакета, а линковка и есть самая прожорливая по памяти фаза. Четыре разом, сотни раз подряд. Хуже того, лекарство от 1-й причины эту усиливает: `go clean -testcache` перед прогоном лишает Go возможности отдать сборку из кеша.

   Отсюда практика: `--workers 1`–`2`, а не 4; `--exclude-files` до тех файлов, ради которых прогон и затеян; и следить за давлением памяти в первые минуты, а не уходить от машины.
